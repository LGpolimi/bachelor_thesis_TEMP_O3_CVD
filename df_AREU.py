...
for file_name in os.listdir(input_path):
    if file_name.endswith('.tab'):
        ...
        if 'MOTIVO_DTL' in df.columns:
            ...
            # Filtra le righe che contengono le parole chiave
...
if dataframes:
    ...
    # Processa la colonna DATA
    if 'DATA' in final_dataframe.columns:
        # Isola solo la parte della data prima dei due punti
        final_dataframe['DATA'] = final_dataframe['DATA'].str.split(':').str[0]

        # Sostituisci i mesi abbreviati con il formato numerico
